...
function Cezpdf($paper='a4',$orientation='portrait'){
	...
	if (!is_array($paper)){
		switch (strtoupper($paper)){
			...
			case 'A41': default: {$size = array(0,0,841.89,595.28); break;}
  ...
  foreach($data as $row){
    ...
    while(!$abortTable && ($newPage || $newRow)){
      ...
      if ($newPage || $y<$this->ez['bottomMargin'] || (isset($options['minRowSpace']) && $y<($this->ez['bottomMargin']+$options['minRowSpace'])) ){
		...
		//implemetar la cabecera del pdf 
		$this->ezCabecera();
	    //$this->ezNewPage();
		/*   *************************************************************************************1516146161464646464646464*/
...
//------------------------------------------------------------------------------
function ezCabecera(){
	 $this->ezText("<b>Relacion de Reserva Conde Travel E.I.R.L. </b>\n\n",16);
	/* $this->ezText("<b>Fecha: </b> ".date("d/m/Y")."\n", 10);
...
{
    // Logo
    //$this->ezImage('logo.png',10,8,33);
    // Arial bold 15
/*    $this->SetFont('Arial','B',15);
    // Movernos a la derecha
    $this->Cell(80);
    // Título
    $this->Cell(30,10,'Relacion de Reservas Conde Travel E.I.R.L.',1,0,'C');
    // Salto de línea
    $this->Ln(20);*/
}
